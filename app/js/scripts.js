...
function is_touch_device(){try{document.createEvent("TouchEvent");return true}catch(e){return false}}if(is_touch_device()){$("#nav-mobile").css({overflow:"auto"})}
// Set checkbox on forms.html to indeterminate
var indeterminateCheckbox=document.getElementById("indeterminate-checkbox");if(indeterminateCheckbox!==null)indeterminateCheckbox.indeterminate=true;
// Pushpin Demo Init
if($(".pushpin-demo-nav").length){$(".pushpin-demo-nav").each(function(){var $this=$(this);var $target=$("#"+$(this).attr("data-target"));$this.pushpin({top:$target.offset().top,bottom:$target.offset().top+$target.outerHeight()-$this.height()})})}
...
if($("#tabs-swipe-demo").length){$("#tabs-swipe-demo").tabs({swipeable:true})}
// Plugin initialization
$(".carousel.carousel-slider").carousel({fullWidth:true});$(".carousel").carousel();$(".slider").slider();$(".parallax").parallax();$(".modal").modal();$(".scrollspy").scrollSpy();$(".button-collapse").sideNav({edge:"left"});$(".datepicker").pickadate({selectYears:20});$("select").not(".disabled").material_select();$("input.autocomplete").autocomplete({data:{Apple:null,Microsoft:null,Google:"http://placehold.it/250x250"}});
// Chips
$(".chips").material_chip();$(".chips-initial").material_chip({readOnly:true,data:[{tag:"Apple"},{tag:"Microsoft"},{tag:"Google"}]});$(".chips-placeholder").material_chip({placeholder:"Enter a tag",secondaryPlaceholder:"+Tag"});$(".chips-autocomplete").material_chip({autocompleteOptions:{data:{Apple:null,Microsoft:null,Google:null}}})})})(jQuery);